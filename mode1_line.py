...
STYLES = {
    "sym_unpinned": dict(color="#0072BD", linestyle="-", marker="o",
                         ...
                         label="Symmetric, no/L"),
    "asy_unpinned": dict(color="#D95319", linestyle="-", label="Asymmetric, no/L"),
    "sym_pinned":   dict(color="#EDB120", linestyle=(0, (5, 2.5)), label="Symmetric, w/L"),
    "asy_pinned":   dict(color="#7E2F8E", linestyle=(0, (1, 1.6)), label="Asymmetric, w/L"),
}
...
# dynamics
def line_laplacian(n: int, epsilon: float, anchor: float = 0.0) -> np.ndarray:
    """Path-graph Laplacian with reflecting ends. Forward edges carry 1 - epsilon,
...
def h2_profile(n: int, epsilon: float, dynamics: str, pinned: bool) -> np.ndarray:
    """Per-gap H2 norm along the platoon."""
    # calibrated closed loop of eq. (Acal): L_alpha <- alpha * L
    L_alpha = ALPHA * line_laplacian(n, epsilon, ALPHA_PIN if pinned else 0.0)
    if not pinned:
    ...
    if dynamics == "RPAV":
        ...
        L_beta = line_laplacian(n, epsilon, BETA_PIN if pinned else 0.0)
        if not pinned:
    ...
    for i in range(n - 1):
        C = np.zeros((1, 2 * n))
        C[0, i] = 1.0
        C[0, i + 1] = -1.0            # no wrap-around: the line has n - 1 gaps
        out[i] = (C @ W @ C.T).item()
    return out
...
def make_figure(n: int, epsilon: float, show_legend: bool = False):
    ...
    if show_legend:
        ...
        fig.legend(handles, labels, loc="lower right", ncol=1, frameon=True,
                   fancybox=False, edgecolor="0.2", framealpha=0.8,
                   fontsize=LEGEND_FONT, borderpad=0.30, labelspacing=0.20,
                   handlelength=2.0, handletextpad=0.3, borderaxespad=0.0,
